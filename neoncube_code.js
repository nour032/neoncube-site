...
// Uses your inline base64 image (INLINE_IMG) for all visuals.
// IMPORTANT: Replace the placeholder below with the FULL data URL string that starts with
// "data:image/webp;base64," followed by a long base64 sequence. If this stays short, images won't render.
const INLINE_IMG = "data:image/webp;base64,UklGRhAAAABXRUJQVlA4WAoAAAAQAAAAMwAAMwAAQUxQSC0AAAABYktHRAC7AADIAAAAABgAAAAAAAAAAAAAAPYAAAD+/////wAAAD+/////wAAAAAAAAAAAAAA..."; // ← replace with full string

// Runtime guard to help debug bad/short data URLs
const isValidDataUrl = (s) => typeof s === "string" && s.startsWith("data:image/") && s.includes("base64,") && s.length > 2000;
...
export default function NeonCubeLanding() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#0a0a0c] text-white selection:bg-blue-500/30">
      {/* Debug banner to surface invalid data URL issues instead of a blank page */}
      {!imgValid && (
