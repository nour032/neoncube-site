...
document.addEventListener('DOMContentLoaded', () => {
    ...
    preorderForm.addEventListener('submit', async(e) => {
        ...
        try {
            ...
            alert('Sending failed. Please send us an email instead.');
        } finally {
...
if (navBurger && navDrawer) {
    ...
    navBurger.addEventListener('click', (e) => {
        e.stopPropagation();
        const willOpen = !navDrawer.classList.contains('active');
        navDrawer.classList.toggle('active', willOpen);
        if (willOpen) {
    ...
    document.addEventListener('click', (e) => {
        if (!navDrawer.contains(e.target) && e.target !== navBurger) {
            closeDrawer();
        }
    });
